...
contract DSCEngine is ReentrancyGuard {
    ...
    */
    ...
    // in order to redeem collateral, you must have a health factor of at least 1
    ...
    * If a user goes below 1, then they can be liquidated.
    ...
    function getAccountCollateralValue(address user) public view returns (uint256 totalCoollateralValueInUsd) {
        for (uint256 i = 0; i < s_collateralTokens.length; i++) {
            address token = s_collateralTokens[i];
            uint256 amount = s_collateralDeposited[user][token];
            totalCoollateralValueInUsd += getUsdValue(token, amount);
        }
        return totalCoollateralValueInUsd;
    }
    ...
    function getTokenAmountFromUsd(address token, uint256 usdAmountwei)
        public
        view
